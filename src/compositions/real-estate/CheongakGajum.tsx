...
const TitleScene: React.FC<(typeof SCENES)[0] & { type: "title" }> = ({
  title,
  sub,
  accent,
}) => {
...
const PointsScene: React.FC<(typeof SCENES)[1] & { type: "points" }> = ({
  title,
  items,
}) => {
  const frame = useCurrentFrame();
      ...
      <div style={{ width: "100%", maxWidth: 1400 }}>
        ...
        {items.map((item, i) => {
          ...
          return (
            ...
            >
              <span style={{ fontSize: 52, fontWeight: 700, color: "#fff" }}>
                {item.label}
              </span>
              <span
                style={{ fontSize: 56, fontWeight: 900, color: item.color }}
              >
                {item.score}
              </span>
            </div>
...
const DetailScene: React.FC<(typeof SCENES)[2] & { type: "detail" }> = ({
  badge,
  badgeColor,
  title,
  items,
        ...
        {items.map((item, i) => {
          ...
          return (
            ...
            >
              ...
              />
              <span style={{ fontSize: 46, color: "#E8EEFF", fontWeight: 500 }}>
                {item}
              </span>
            </div>
          );
...
const TrapScene: React.FC<(typeof SCENES)[5] & { type: "trap" }> = ({
  title,
  traps,
}) => {
  const frame = useCurrentFrame();
        ...
        {traps.map((trap, i) => {
          ...
          return (
            ...
            >
              ...
              <span style={{ fontSize: 46, color: "#fff", fontWeight: 600 }}>
                {trap.text}
              </span>
            </div>
          );
...
const CtaScene: React.FC<(typeof SCENES)[6] & { type: "cta" }> = ({
  rows,
  next,
}) => {
  const frame = useCurrentFrame();
